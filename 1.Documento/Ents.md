...
| position_key          | string   | sim         | ordenação lexical (LexoRank) no dia             |
| version               | int      | sim         | controle de concorrência                        |
| criado_em             | datetime | sim         |                                                 |
| atualizado_em         | datetime | sim         |                                                 |

Notas:
• `position_key` (String) existe para suportar reorder via LexoRank (fractional-indexing).
...
• `client_type` define vínculo com Cliente: - Se 'WalkIn': `cliente_id` deve ser nulo e `walk_in_name` informado. - Se 'Person': `cliente_id` obrigatório e `walk_in_name` nulo.
