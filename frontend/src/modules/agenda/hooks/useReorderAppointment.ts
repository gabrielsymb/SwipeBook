...
interface ReorderMutationPayload extends ReorderAppointmentRequestDTO {
  movedId: string;
  date: Date; // A data do dia sendo reordenado
  newOrderIds: string[]; // A nova ordem de IDs resultante do Drag & Drop (para atualização otimista)
}
...
export function useReorderAppointment() {
  ...
  const mutation = useMutation<
    void, // Retorno de Sucesso (vazio)
    ApiErrorResponse, // Retorno de Erro
    ReorderMutationPayload, // Payload de Entrada
    { previousAppointments: Agendamento[] | undefined } // Contexto retornado por onMutate
  >({
    ...
    onMutate: async (payload) => {
      ...
      const previousAppointments =
        queryClient.getQueryData<Agendamento[]>(queryKey);
    ...
    onError: (_err, payload, context) => {
      ...
      if (context?.previousAppointments) {
        ...
        queryClient.setQueryData(
          [AGENDA_QUERY_KEY, formattedDate],
