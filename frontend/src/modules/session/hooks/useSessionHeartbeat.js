 ...
 */
export function useSessionHeartbeat(options = {}) {
    ...
    const lastSyncRef = useRef(undefined);
    // 1. MUTAÇÃO: Função para enviar o Heartbeat (uso interno)
    const heartbeatMutation = useMutation({
        mutationFn: ({ sessionId, elapsedMs }) => sessionApi.sendHeartbeat(sessionId, { elapsedMs }),
